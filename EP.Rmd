...
Ao realizarmos os testes, percebemos que nossa distribuição aparenta ter caracteristicas e formas de algumas distribuições conhecidas, sendo elas: Poisson, Exponencial e Geométrica. Neste caso vamos tentar identificar qual distribuição é mais similar a nossa distribuição e qual o melhor paramêtros da distribuição escolhida.

Para encontrar a melhor distribuição da nossa distribuição usamos o pacote **fitdistrplus** que dado o conjunto de observações e a função de densidade de probabilidade ele retorna qual o desvio padrão das observações contra os valores esperados da distribuição.

Sendo assim, realizamos a simulação e obtemos os seguintes valores:

              | Poisson | Exponencial | Geométrica
------------------------|-------------| -------------
Desvio Padrão | Cell 2  |             |

* Vale ressaltar que a simulação foi realizada com os seguintes parâmetros:

- Número de vértices: 10
- Número de simulações: 1000
- Probabilidade de ligar o vértice: 0.4

Com isso, escolhemos a distribuição geométrica como hipótese de distribuição para nossa simulação.Em seguida vamos encontrar o melhor parâmetro p, considerando que $X\sim Geo(p)$.
...
Usaremos essa premissa para encontrar os parâmetros da nossa distribuição geométrica.
Sendo assim devemos conhecer a esperança da distribuição geométrica:

$$E(X\sim Geo(p)) = \frac{1}{p}$$ 


Em seguida iremos realizar o teste de $\chi^2$ para avaliarmos se estamos cometendo o erro da escolha da nossa distruibuição 

#Observações:
Nosso processo de simulação é um processo discreto, além disso realizamos algumas mudanças em nossas distribuições de T simulada. Primeiramente, retiramos a observação 2 de nosso processo, porque o problema definido não permite que o tempo minimo de volta seja 2 levando assim: $O_{2} = 0 $, retiramos também a oberservação 1 por dois motivos. 1º Ela distorce a distribuição que gostaríamos que fosse, 2º de fato $P(O_{1}) = p$, como o problema é definido para podermos voltar ao mesmo nó com probabilidade $p$ , fica bem definido assim $P(O_{1}) = p$
e por fim e não menos importante, retiramos $T_{inf}$ porque ele inviesa nossa distribuição esperada. 

##Teste de aderência:
Com nossa função proposta em mãos, podemos realizar o teste $\chi^2$ para avaliarmos se estamos ou não comentendo o erro de escolher a distribuição Geométrica com parâmetro(p=)

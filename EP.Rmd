...
Ao realizarmos os testes, percebemos que a nossa aparenta ter formas de algumas distribuições conhecidas, sendo elas: Poisson, Exponencial e Geométrica. Neste caso vamos tentar identificar quais paramêtros de cada uma destas distribuições.
Com isso queremos encontrar a distribuição que possui a menor distância com nossa distribuição simulada, isto é:

$$ \sum_{i=1}^{N}|T_{i} - P(X_{d} = i)|, d_{1}:=Poisson;d_{2}:=Exponencial;d_{3}:= Geométrica$$
...
Usaremos essa premissa para encontrar os parâmetros da nossa distribuição.
Sendo assim devemos conhecer as esperanças das distribuições: Exponencial, Geométrica e de Possion:

$$E(X\sim Geo(p)) = \frac{1}{p} \\ E(X\sim Exp(\lambda)) = \frac{1}{\lambda} \\ E(X\sim Pois(\lambda)) = \frac{1}{\lambda}$$

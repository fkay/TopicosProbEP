...
A distribuição sem as observações citadas tem uma distribuição reescalada na forma:
...
#geo = 1/lambd
plot(c(NA,NA,dTp), x = 1:nd, col="blue", type = "b", xlab="Nº de Vertices", ylab="Probabilidade")
...
grid(nx = NULL, ny = NULL, col = "darkgray", lty = "dotted", lwd = par("lwd"), equilogs = TRUE)
legend(x = "topright", col = c("blue", "purple", "black", "green"), 
         legend = c("Simulado", "Dist. Exponencial", "Dist. Geométrica", "Dist. Poisson"),
         fill = c("blue", "purple", "black", "green"))
```
...
**Hipótese nula:** Distribuição Geométrica com parâmetro $p=`r geo`$ seja igual a distribuição T simulada.
**Hipótese alternativa:**  Distribuição Geométrica com parâmetro $p=`r geo`$ seja Diferente a distribuição T simulada.
...
```{r dist_completa_T, echo=FALSE}
plot(c(prob,0,dTP,1-sum(dTP)- prob), x = 1:(nd+1), col="green", type = "b", xlab="Nº de Vertices", ylab="Probabilidade")
...
## Premissas
De forma análoga a distribuição de T, retiramos desta vez apenas a observação 1, ou seja $C(v,v') = 1$, porque $P(C=1) = p = `r prob`$.
